...
$APPLICATION->SetTitle(Loc::getMessage('MAKE_AN_ORDER_PAGE_TITLE'));
...
<?php global $USER;
	if (!$USER->IsAuthorized()) {
		...
		Array(
			...
			"PROPERTY_CODE" => array("ORDERED_PRODUCTS","CLIENT","STATUS",""),
			"SET_BROWSER_TITLE" => "Y",
			"SET_LAST_MODIFIED" => "N",
					...
					<?php if (isset($_SESSION['BASKET']['PRODUCTS']) && !empty($_SESSION['BASKET']['PRODUCTS'])):?>
						<?php foreach ($_SESSION['BASKET']['PRODUCTS'] as $name => $props):?>
							<tr class="order__table-third__line">
								...
								<td class="order__table-third__column">
									<?=$props['PRICE'];?> <?=Loc::getMessage('RUB');?>
								</td>
								<td class="order__table-third__column">
									<?=$props['SUM'];?>
								</td>
			...
			<div class="order__block-bottom">
				<div class="order__block-bottom__text">
					...
					<h3 class="order__price" id="order-sum" style="font-family: DIN Condensed;"><?php echo isset($_SESSION['BASKET']['TOTAL_SUM']) && !empty($_SESSION['BASKET']['TOTAL_SUM']) ? $_SESSION['BASKET']['TOTAL_SUM'] : 0;?> <?=Loc::getMessage('RUB');?></h3>
				</div>
				<div class="order__button-gray" <?php if (isset($_SESSION['BASKET']['PRODUCTS']) && (!empty($_SESSION['BASKET']['PRODUCTS']))) echo 'id="order-btn"'; else echo 'style="cursor: default;"';?>>
					<p class="order__button__text" <?php if (isset($_SESSION['BASKET']['PRODUCTS']) && (!empty($_SESSION['BASKET']['PRODUCTS']))) echo 'id="order-button"';?>>
						<?=Loc::getMessage('ORDER');?>
					</p>
					<svg id="color-change-svg-order" xmlns="http://www.w3.org/2000/svg" width="43" height="43" fill="#4A4A4A">
